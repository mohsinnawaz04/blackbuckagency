...
const Modal = ({ isModalOpen, toggleModal }) => {
  return (
    <div
      className={`modal fixed p-4 sm:p-8 pb-14 w-full max-w-[95%] sm:max-w-lg rounded-3xl z-50 top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 transition-all duration-500 ease-in-out bg-white shadow-lg ${
        isModalOpen
          ? "opacity-100 rotate-y-0 scale-100"
          : "opacity-0 rotate-y-90 scale-75"
      }`}
      style={{ transformStyle: "preserve-3d", maxHeight: "100vh" }}
    >
      <div className="overflow-y-auto max-h-[85vh] pr-2 sm:max-h-full">
        <h2 className="text-3xl mb-5 max-w-[90%]">Get Started</h2>
        <button
          onClick={toggleModal}
          className="close_modal absolute top-1 right-1 rounded-full p-[7px] cursor-pointer z-50"
        >
